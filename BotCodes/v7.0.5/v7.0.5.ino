...
#define SENSING_THRESHOLD  0.95  // to normalize for different environments
...
#define DIR 11   // DIR pin of the motor drive
#define STEP 10  // Step pin of the motor drive
#define ENBL 12  // En pin of the motor drive
...
#define RANDOM_MOVE_TURNS 4
#define GET_STABLE_TIME 5000

//2 relay module
#define STEPPER_MOTOR 9
#define PROPELLER_MOTORS 8
...
//  char text[] = ""+"Yo wassup, haha";
  char botID[] = "00000001";
  char XORkey[] = "0000000000000000000000000000000";
  char sendingMsgBeforeEncrypt[32] = {0};
  ...
  char ignorebit = "0";
  strcat(sendingMsgBeforeEncrypt, botID);
  strcat(sendingMsgBeforeEncrypt, fishflag);
  strcat(sendingMsgBeforeEncrypt, GPSLocation);
  strcat(sendingMsgBeforeEncrypt, movedToNewLocation);
  strcat(sendingMsgBeforeEncrypt, DamageControlState);
  strcat(sendingMsgBeforeEncrypt, ignorebit);
...
//  }
  int i = 0;
  for(i = 0; i < 32; i++){
    sendingMsg[i] = XOR(sendingMsgBeforeEncrypt[i], XORkey[i]);
  }
