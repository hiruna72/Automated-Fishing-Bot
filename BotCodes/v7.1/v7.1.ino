...
//  char text[] = ""+"Yo wassup, haha";
  char botID[] = "0000001";
  char sendingMsgTemp[32] = {0};
  char fishflag[2];
  itoa(caughtfishflag, fishflag, 10);
  char GPSLocation[] = "00000000000000000010";
  ...
  char evenparity;
  char XORKey[32] = {0};
  strcat(sendingMsgTemp, botID);
  strcat(sendingMsgTemp, fishflag);
  strcat(sendingMsgTemp, GPSLocation);
  strcat(sendingMsgTemp, movedToNewLocation);
  strcat(sendingMsgTemp, DamageControlState);
  //strcat(sendingMsgTemp, ignorebit);
  int i = 0;
  int noOfOnes = 0;
  for(i = 0; i < 31; i++){
    if(sendingMsgTemp[i] == '1'){
      noOfOnes++;
    }
  }
  if(noOfOnes % 2 == 1){
  ...
  sendingMsg[0] = evenparity;
  strcat(sendingMsg, sendingMsgTemp);
